...
function makeExpectedList(users, list, ideas = []) {
  const user = users.find((user) => user.id === list.user_id);

  const number_of_ideas = ideas.filter((idea) => idea.list_id === list.id)
    .length;
...
function makeExpectedListIdeas(users, listId, ideas) {
  const expectedIdeas = ideas.filter((idea) => idea.list_id === listId);
...
function seedUsers(db, users) {
  ...
  return db
    ...
    .then(() =>
      ...
      db.raw(`SELECT setval('fmn_users_id_seq', ?)`, [
...
function seedListsTables(db, users, lists, ideas = []) {
  ...
  return db.transaction(async (trx) => {
    ...
    await trx.raw(`SELECT setval('fmn_lists_id_seq', ?)`, [
    ...
    if (ideas.length) {
      ...
      await trx.raw(`SELECT setval('fmn_ideas_id_seq', ?)`, [
...
function seedMaliciousIdea(db, user, idea) {
  return seedUsers(db, [user]).then(() => db.into("fmn_ides").insert([idea]));
}
