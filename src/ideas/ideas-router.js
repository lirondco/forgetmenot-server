...
ideasRouter
  ...
  .all(requireAuth)
  .all((req, res, next) => {
    IdeasService.getById(req.app.get("db"), req.params.idea_id)
      .then((idea) => {
        if (!idea)
  ...
  .patch(jsonBodyParser, (req, res, next) => {
    ...
    IdeasService.updateIdea(req.app.get("db"), req.params.idea_id, newFields)
      .then(() => {
        res.status(204).end();
      })
      .catch(next);
  ...
  .delete((req, res, next) => {
    ...
    IdeasService.deleteIdea(req.app.get("db"), req.params.idea_id)
      .then(() => {
        res.status(204).end();
      })
      .catch(next);
